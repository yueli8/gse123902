...
features = c("CD79A","IL32","TRAC","NKG7","PRF1","FGFBP2","GZMH",
             "IL7R","TPM2","KRT19","CAV1", "FTL", "GATA2", "LST1", "LDHB",
              "CD8A",  "RGCC","CD3D")
DotPlot(hms_cluster,features=features)+RotatedAxis()  
features = c("FTH1","FTL","HLA-DRA",
 "PRF1",  "CCL5", "NKG7" )
FeaturePlot(hms_cluster,features=features,ncol=3)

setwd("~/gse123902/CytoTRACE")
hms_cluster<-readRDS("p8_cluster_id.rds")
DimPlot(hms_cluster, reduction = "umap", label = TRUE, pt.size = 0.5) 
DimPlot(hms_cluster, reduction = "umap", label = FALSE, pt.size = 0.5) 
features = c("CD79A","CCL5", "TPM2",
            "KRT19", "CAV1", "FTL",  "CD8A",  "ANXA1", 
           "KLRD1","PGAP1", "AIF1",
            "SCGB1A1", "CXCL13", "GNLY", "IFI6",
            "ISG15", "CD69",  "DUSP4")
DotPlot(hms_cluster,features=features)+RotatedAxis() 
features = c("TMSB10","ACTB","FTL",
        "IL7R", "BTG1", "CXCR4")
FeaturePlot(hms_cluster,features=features,ncol=3)
...
features = c("COL1A2","CD74","IL32",
   ...
   "RGS1","PLCG2","CXCR4","JUNB","ZFP36L2" )
DotPlot(hms_cluster,features=features)+RotatedAxis() 
features = c("FTH1","FTL","IGHA1")
FeaturePlot(hms_cluster,features=features,ncol=3)
